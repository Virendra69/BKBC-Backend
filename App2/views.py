...
def getBlessingCardAPI(request):
    ...
    try:
        ...
        return JsonResponse({"blessing_card_base64": blessing_card_base64})
    except:
        return JsonResponse({"blessing_card_base64": ""})
...
def sendEmailAPI(request):
    try:
        ...
        email.attach("blessing_card.png", image_data, "image/png")
        # Send the email
        email.send()
...
def getCard(request, token):
    try:
        ...
        return JsonResponse({"base_64_image": base_64_image})
    except TokenPathMapping.DoesNotExist:
        return JsonResponse({'error_message': 'Image not found'})